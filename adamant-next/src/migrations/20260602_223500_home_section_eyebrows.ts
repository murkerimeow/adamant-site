...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  for (const field of fields) {
    await addColumn(db, "home_page", `section_eyebrows_${field}`, "text");
    await addColumn(db, "_home_page_v", `version_section_eyebrows_${field}`, "text");
  }
}

export async function down({ db }: MigrateDownArgs): Promise<void> {
  for (const field of [...fields].reverse()) {
    await dropColumn(db, "_home_page_v", `version_section_eyebrows_${field}`);
    await dropColumn(db, "home_page", `section_eyebrows_${field}`);
  }
}
