...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await addColumn(db, "catalog", "seo_title", "text");
  await addColumn(db, "catalog", "seo_description", "text");
  await addColumn(db, "_catalog_v", "version_seo_title", "text");
  await addColumn(db, "_catalog_v", "version_seo_description", "text");
}

export async function down({ db }: MigrateDownArgs): Promise<void> {
  await dropColumn(db, "_catalog_v", "version_seo_description");
  await dropColumn(db, "_catalog_v", "version_seo_title");
  await dropColumn(db, "catalog", "seo_description");
  await dropColumn(db, "catalog", "seo_title");
}
